import React, { useState } from 'react';
import { base44 } from '@/api/base44Client';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { X, Zap, Clock } from 'lucide-react';
import { toast } from 'sonner';

const STATUSES = ['new', 'cold_outreach', 'contacted', 'qualified', 'in_negotiation', 'converted', 'unresponsive'];

export default function SequenceEditor({ sequence, templates, onClose, onSaved }) {
  const [form, setForm] = useState({
    name: sequence?.name || '',
    trigger_type: sequence?.trigger_type || 'status_change',
    trigger_status: sequence?.trigger_status || 'cold_outreach',
    delay_days: sequence?.delay_days ?? 0,
    template_id: sequence?.template_id || (templates[0]?.id || ''),
    is_active: sequence?.is_active ?? true,
    filter_has_email: sequence?.filter_has_email || false,
    filter_has_phone: sequence?.filter_has_phone || false,
    filter_category: sequence?.filter_category || '',
  });
  const [saving, setSaving] = useState(false);

  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));

  const save = async () => {
    if (!form.name.trim()) { toast.error('Name is required'); return; }
    if (!form.template_id) { toast.error('Please select a template'); return; }
    setSaving(true);
    const tpl = templates.find(t => t.id === form.template_id);
    const data = { ...form, template_name: tpl?.name || '' };
    if (sequence?.id) {
      await base44.entities.FollowUpSequence.update(sequence.id, data);
    } else {
      await base44.entities.FollowUpSequence.create(data);
    }
    toast.success(sequence ? 'Sequence updated' : 'Sequence created');
    onSaved();
  };

  const selStyle = (active) => ({
    background: active ? 'rgba(248,212,23,0.15)' : 'rgba(10,25,41,0.5)',
    border: `2px solid ${active ? '#f8d417' : '#5e6a78'}`,
    color: active ? '#f8d417' : '#9ea7b5'
  });

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.7)' }}>
      <div className="w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden" style={{
        background: 'linear-gradient(135deg, #0a1929 0%, #1a2332 100%)', border: '2px solid #f8d417'
      }}>
        <div className="flex items-center justify-between p-5 border-b" style={{ borderColor: 'rgba(248,212,23,0.3)' }}>
          <h2 className="text-lg font-bold" style={{ color: '#f8d417', fontFamily: 'Poppins, sans-serif' }}>
            {sequence ? 'Edit Sequence' : 'New Sequence'}
          </h2>
          <button onClick={onClose} style={{ color: '#9ea7b5' }}><X className="h-5 w-5" /></button>
        </div>

        <div className="p-5 space-y-5 overflow-y-auto" style={{ maxHeight: '70vh' }}>
          <div>
            <label className="text-xs font-semibold mb-1 block" style={{ color: '#9ea7b5' }}>Sequence Name</label>
            <Input value={form.name} onChange={e => set('name', e.target.value)} placeholder="e.g., Welcome New Lead"
              style={{ background: '#071a2c', borderColor: '#f8d417', color: '#fff' }} />
          </div>

          <div>
            <label className="text-xs font-semibold mb-2 block" style={{ color: '#9ea7b5' }}>Trigger Type</label>
            <div className="grid grid-cols-2 gap-2">
              <button onClick={() => set('trigger_type', 'status_change')}
                className="flex items-center justify-center gap-2 py-3 rounded-xl font-semibold text-sm transition-all"
                style={selStyle(form.trigger_type === 'status_change')}>
                <Zap className="h-4 w-4" /> Status Change
              </button>
              <button onClick={() => set('trigger_type', 'time_based')}
                className="flex items-center justify-center gap-2 py-3 rounded-xl font-semibold text-sm transition-all"
                style={selStyle(form.trigger_type === 'time_based')}>
                <Clock className="h-4 w-4" /> Time-Based
              </button>
            </div>
          </div>

          {form.trigger_type === 'status_change' && (
            <div>
              <label className="text-xs font-semibold mb-2 block" style={{ color: '#9ea7b5' }}>Trigger on Status Change To</label>
              <div className="flex flex-wrap gap-2">
                {STATUSES.map(s => (
                  <button key={s} onClick={() => set('trigger_status', s)}
                    className="text-xs px-3 py-1.5 rounded-full transition-all font-medium"
                    style={{
                      background: form.trigger_status === s ? 'rgba(74,203,191,0.2)' : 'transparent',
                      border: `1px solid ${form.trigger_status === s ? '#4acbbf' : '#5e6a78'}`,
                      color: form.trigger_status === s ? '#4acbbf' : '#9ea7b5'
                    }}>
                    {s.replace(/_/g, ' ')}
                  </button>
                ))}
              </div>
            </div>
          )}

          <div>
            <label className="text-xs font-semibold mb-1 block" style={{ color: '#9ea7b5' }}>Delay (days) — 0 = send immediately</label>
            <Input type="number" min="0" max="365" value={form.delay_days}
              onChange={e => set('delay_days', Number(e.target.value))}
              style={{ background: '#071a2c', borderColor: '#54b0e7', color: '#fff' }} />
          </div>

          <div>
            <label className="text-xs font-semibold mb-2 block" style={{ color: '#9ea7b5' }}>Template to Send</label>
            {templates.length === 0 ? (
              <p className="text-xs" style={{ color: '#f66c25' }}>No templates yet — create one first</p>
            ) : (
              <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
                {templates.map(t => (
                  <button key={t.id} onClick={() => set('template_id', t.id)}
                    className="w-full text-left px-3 py-2.5 rounded-lg transition-all text-sm"
                    style={{
                      background: form.template_id === t.id ? 'rgba(74,203,191,0.15)' : 'rgba(10,25,41,0.5)',
                      border: `1px solid ${form.template_id === t.id ? '#4acbbf' : '#5e6a78'}`,
                      color: form.template_id === t.id ? '#4acbbf' : '#9ea7b5'
                    }}>
                    <span className="font-medium">{t.name}</span>
                    <span className="ml-2 text-xs opacity-60">[{t.channel}]</span>
                  </button>
                ))}
              </div>
            )}
          </div>

          <div>
            <label className="text-xs font-semibold mb-2 block" style={{ color: '#9ea7b5' }}>Lead Filters (optional)</label>
            <div className="space-y-2">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={form.filter_has_email} onChange={e => set('filter_has_email', e.target.checked)} className="accent-cyan-400" />
                <span className="text-sm" style={{ color: '#d7dde5' }}>Only leads with email</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={form.filter_has_phone} onChange={e => set('filter_has_phone', e.target.checked)} className="accent-cyan-400" />
                <span className="text-sm" style={{ color: '#d7dde5' }}>Only leads with phone</span>
              </label>
              <div className="flex items-center gap-2">
                <span className="text-sm flex-shrink-0" style={{ color: '#d7dde5' }}>Category:</span>
                <Input value={form.filter_category} onChange={e => set('filter_category', e.target.value)}
                  placeholder="Leave blank for all"
                  style={{ background: '#071a2c', borderColor: '#5e6a78', color: '#fff', height: '32px', fontSize: '12px' }} />
              </div>
            </div>
          </div>
        </div>

        <div className="flex gap-3 p-5 border-t" style={{ borderColor: 'rgba(248,212,23,0.3)' }}>
          <Button onClick={onClose} variant="ghost" className="flex-1" style={{ color: '#9ea7b5', border: '1px solid #5e6a78' }}>Cancel</Button>
          <Button onClick={save} disabled={saving} className="flex-1 font-semibold"
            style={{ background: 'linear-gradient(135deg, #f8d417 0%, #f66c25 100%)', color: '#0a1929' }}>
            {saving ? 'Saving...' : (sequence ? 'Update' : 'Create Sequence')}
          </Button>
        </div>
      </div>
    </div>
  );
}